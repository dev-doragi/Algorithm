...
// ptr의 next는 node2이다. ptr->next가 무조건 마지막 노드를 가리키는것이 아니다. ptr은 시작 주소!!!
void append(simple_pointer ptr, simple_pointer inode) { // ptr의 다음 노드로 inode를 추가하는 함수
	...
	// ptr의 마지막 주소가 NULL인데, 이를 inode의 시작 주소로 바꾸기 위해선, ptr의 마지막 노드의 링크값을 수정해야 한다.
	// 현재 ptr은 ptr의 시작 주소이기 때문에 while문을 통해 ptr의 각 노드를 before에 저장해서 ptr의 마지막 노드를 before에 저장한다.
	while (ptr != NULL) { // ptr이 NULL을 가리키고 있지 않을 때,
		before = ptr; // before은 ptr의 마지막 노드를 가리킨다.
		ptr = ptr->next; 
	}
	// while문이 끝나면, before는 ptr의 마지막 노드를 가리킨다.
